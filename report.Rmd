...
When looking directly at the accuracy of predicting the estimated treatment effect($\beta$), we provide the mean of estimated $\beta$ with standard deviation by 1000 times simulation to compare with the true $\beta$ we assumed as 0.5(Table. 2). Exponential proportional-hazard model only performs well when the original dataset generated by exponential distribution as baseline function. The Weibull proportional-hazard model could predict accurately when the training datasets are simulated by both exponential and Weibull distribution as baseline function in that the estimated $\beta$ is close to 0.5. As for the Cox proportional-hazard model, it can predict the estimated $\beta$ most accurately - all $\beta$ are close to 0.5 regardless of the original training data based on different baseline functions. We also calculated the bias of the estimated $\beta$, which is defined as the difference between the estimated $\beta$ and the true $\beta$. Results shown in Table.3 give the similar results as the previous tables. 

# Conclusions & Discussion 
From the results above, we find that in general, survival models have the best performance under their corresponding baseline hazard functions. For example, from the leftmost column in Figure 1, we can see that the fitted exponential curves deviate from the K-M curves when the baseline is not exponentially distributed. This finding can also be confirmed numerically: the exponential survival model has a small MSE when the simulated dataset follows an exponential distribution, but the MSE grows large (worse performance) otherwise. In other words, the more generalized the model, the more robust it will be when the baseline hazard functions go misspecified. Combining our findings from the graph and the tables, we can see that the Cox model has small MSE, small bias with little variance across different underlying datasets. Given that MSE is the  Thus, we concluded that the Cox proportional hazard model, in general, has the best performance among the three survival models in terms of accuracy, efficiency, and robustness to misspecified baseline hazard functions. 

It should be noted that these three survival models all have their advantages and disadvantages. For Exponential and Weibull distribution, both of them can estimate survival function $S(t)$ and hazard ratio (HR). However, for the exponential distribution, it’s not always realistic to assume that $h_0(t)$ is a constant. This problem went away in the Weibull distribution as it allows the hazard to increase or decrease proportionally with time. Nevertheless, since these two models are both parametric, which assumes that the underlying population distribution has been correctly specified. Thus, they lack robustness when the baseline hazard function went misspecified. As for the Cox proportional hazard model, it is more robust than the previous two, as we don’t need to specify the baseline hazard function. However, this semi-parametric approach sacrifices its ability to estimate $S(t)$ and HR for its robustness.

There are still some limitations of our simulations. First of all, we didn’t simulate censoring in our data and made the assumption that we’re able to observe all the outcomes of each subject. This quickly becomes impossible in the realistic setting where losses to follow-ups are common. Second, out of numerous baseline hazard functions, we only choose three of them. Though we can still reach a relatively definite conclusion, we can still explore more in the future whether the Cox model is still robust to other baseline hazard functions. 
